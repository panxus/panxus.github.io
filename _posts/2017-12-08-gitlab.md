...
0. 本次测试需两台虚拟机,一台搭gitlab，一台搭项目服务器。

1. 基础依赖软件安装[可参照gitlab中文网]
...
8.初始化需要配置ssh 公钥。生成ssh密钥命令.
```
ssh-keygen -t rsa -C "panxu94s@163.com"
```

9. gitlab master 默认 protected，在项目分支管理下解除master protected，不然push会失败，错误提示如下。
```
